...
export { registration, login, getUserData, changePassword, getUsers, removeUser, getUserActivity, auth0Registration, forgotPassword, resetPassword }

// Step 1 of password reset: generate a one-time token, email a reset link.
...
const forgotPassword = async (req, res) => {
  try {
    ...
    if (!email || !email.trim()) {
      return res.status(HttpStatus.BAD_REQUEST).json({ status: false, message: "Email is required" });
    }
    ...
    if (user) {
      ...
      await sendEmail({
        to: user.email,
        subject: "Reset your Smart Post password",
        text: `You requested a password reset. This link expires in 1 hour: ${resetUrl}\n\nIf you didn't request this, you can safely ignore this email.`,
        html: `<p>You requested a password reset. This link expires in 1 hour:</p><p><a href="${resetUrl}">${resetUrl}</a></p><p>If you didn't request this, you can safely ignore this email.</p>`,
      });
    }

    // Same response whether or not a matching user was found.
    return res.status(HttpStatus.OK).json({ status: true, message: genericMessage });
  } catch (error) {
    console.error(error);
    res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ status: false, message: ToastyConstant.SERVER.INTERNAL_SERVER_ERROR });
  }
...
// Step 2 of password reset: verify the token from the link and set a new password.
const resetPassword = [
  check('newPassword')
    .notEmpty()
    .withMessage('New password is required')
  ...
  async (req, res) => {
    try {
      ...
      if (!token) {
        return res.status(HttpStatus.BAD_REQUEST).json({ status: false, message: "Reset token is required" });
      }
      ...
      if (!user) {
        return res.status(HttpStatus.BAD_REQUEST).json({ status: false, message: "This reset link is invalid or has expired. Please request a new one." });
      }
      ...
      return res.status(HttpStatus.OK).json({ status: true, message: "Password reset successfully. You can now log in with your new password." });
    } catch (error) {
      console.error(error);
      res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ status: false, message: ToastyConstant.SERVER.INTERNAL_SERVER_ERROR });
    }
