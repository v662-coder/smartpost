...
const addPost = async (req, res) => {
    try {
        ...
        if (!title || !title.trim() || !tags.length || !description || !description.trim()) {
            return res.status(HttpStatus.BAD_REQUEST).json({ status: false, message: "Title, at least one tag, and description are all required" });
        }
        ...
        if (savedPost) {
            return res.status(HttpStatus.CREATED).json({ status: true, message: "Post created successfully", post: savedPost });
        } else {
